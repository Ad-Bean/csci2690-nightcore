  ...
  namespace utils {
    ...
    class AppendableBuffer {
     public:
      ...
      explicit AppendableBuffer(int initial_size = kDefaultInitialSize) : buf_size_(initial_size), pos_(0) {
      ...
      void AppendData(std::span<const char> data) { AppendData(data.data(), data.size()); }
      ...
      std::span<const char> to_span() const { return std::span<const char>(buf_, pos_); }
    ...
    template <class T>
    void ReadMessages(AppendableBuffer* buffer, const char* new_data, size_t new_data_length,
