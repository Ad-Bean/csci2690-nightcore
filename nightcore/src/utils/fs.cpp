...
namespace faas {
  namespace fs_utils {
    ...
    namespace {
      ...
      int RemoveFileFtwFn(const char* fpath, const struct stat* sb, int typeflag, struct FTW* ftwbuf) {
    ...
    bool RemoveDirectoryRecursively(std::string_view path) {
      return nftw(std::string(path).c_str(), RemoveFileFtwFn, 8, FTW_DEPTH | FTW_MOUNT | FTW_PHYS) == 0;
    }
