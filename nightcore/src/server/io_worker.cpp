...
namespace faas {
  namespace server {
    ...
    IOWorker::IOWorker(std::string_view worker_name, size_t read_buffer_size, size_t write_buffer_size)
        : worker_name_(worker_name),
          state_(kCreated),
          log_header_(fmt::format("{}: ", worker_name)),
          event_loop_thread_(fmt::format("{}/EL", worker_name), absl::bind_front(&IOWorker::EventLoopThreadMain, this)),
      ...
      UV_DCHECK_OK(uv_async_init(&uv_loop_, &run_fn_event_, &IOWorker::RunScheduledFunctionsCallback));
    ...
    void IOWorker::Start(int pipe_to_server_fd) {
      ...
      UV_DCHECK_OK(uv_read_start(UV_AS_STREAM(&pipe_to_server_), &PipeReadBufferAllocCallback,
    ...
    void IOWorker::OnConnectionClose(ConnectionBase* connection) {
      ...
      if (connection->type() >= 0) {
        ...
        HLOG(INFO) << fmt::format("One connection of type {0} closed, total of type {0} is {1}", connection->type(),
      ...
      UV_DCHECK_OK(uv_write(write_req, UV_AS_STREAM(&pipe_to_server_), &uv_buf, 1, &IOWorker::PipeWriteCallback));
    }
    ...
    UV_READ_CB_FOR_CLASS(IOWorker, NewConnection) {
      ...
      if (connection->type() >= 0) {
        ...
        HLOG(INFO) << fmt::format("New connection of type {0}, total of type {0} is {1}", connection->type(),
                                  connections_by_type_[connection->type()].size());
      }
      if (state_.load(std::memory_order_consume) == kStopping) {
    ...
    UV_WRITE_CB_FOR_CLASS(IOWorker, PipeWrite) {
      ...
      connections_on_closing_--;
      if (state_.load(std::memory_order_consume) == kStopping && connections_.empty() && connections_on_closing_ == 0) {
    ...
    UV_ASYNC_CB_FOR_CLASS(IOWorker, RunScheduledFunctions) {
      ...
      if (async_event_recv_timestamp != 0) {
        uv_async_delay_stat_.AddSample(
            gsl::narrow_cast<int32_t>(GetMonotonicMicroTimestamp() - async_event_recv_timestamp));
      }
