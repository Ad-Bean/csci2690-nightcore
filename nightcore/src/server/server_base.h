  ...
  namespace server {
    ...
    class ServerBase : public uv::Base {
     ...
     protected:
      ...
      IOWorker* CreateIOWorker(std::string_view worker_name, size_t read_buffer_size = kDefaultIOWorkerBufferSize,
                               size_t write_buffer_size = kDefaultIOWorkerBufferSize);
      void RegisterConnection(IOWorker* io_worker, ConnectionBase* connection, uv_stream_t* uv_handle);
