...
namespace faas {
  namespace server {
    ...
    IOWorker* ServerBase::CreateIOWorker(std::string_view worker_name, size_t read_buffer_size,
      ...
      UV_CHECK_OK(uv_read_start(UV_AS_STREAM(pipe_to_worker), &PipeReadBufferAllocCallback,
    ...
    void ServerBase::RegisterConnection(IOWorker* io_worker, ConnectionBase* connection, uv_stream_t* uv_handle) {
      ...
      *buf = connection;
      ...
      UV_DCHECK_OK(
          uv_write2(write_req, UV_AS_STREAM(pipe_to_worker), &uv_buf, 1, UV_AS_STREAM(uv_handle), &PipeWrite2Callback));
    }
    ...
    UV_READ_CB_FOR_CLASS(ServerBase, ReturnConnection) {
      if (nread < 0) {
        ...
        utils::ReadMessages<ConnectionBase*>(&return_connection_read_buffer_, buf->base, nread,
                                             [this](ConnectionBase** connection) { OnConnectionClose(*connection); });
      }
      free(buf->base);
    }
