...
namespace faas {
  namespace uv {
    ...
    Subprocess::Subprocess(std::string_view cmd, size_t max_stdout_size, size_t max_stderr_size)
        : state_(kCreated), cmd_(cmd), max_stdout_size_(max_stdout_size), max_stderr_size_(max_stderr_size), pid_(-1) {
    ...
    bool Subprocess::Start(uv_loop_t* uv_loop, utils::BufferPool* read_buffer_pool, ExitCallback exit_callback) {
      ...
      if (std_fds_[kStdout] == -1) {
        UV_DCHECK_OK(uv_read_start(UV_AS_STREAM(&uv_pipe_handles_[kStdout]), &Subprocess::BufferAllocCallback,
                                   &Subprocess::ReadStdoutCallback));
      }
      if (std_fds_[kStderr] == -1) {
        UV_DCHECK_OK(uv_read_start(UV_AS_STREAM(&uv_pipe_handles_[kStderr]), &Subprocess::BufferAllocCallback,
                                   &Subprocess::ReadStderrCallback));
      }
    ...
    UV_READ_CB_FOR_CLASS(Subprocess, ReadStdout) {
      ...
      if (nread < 0) {
        if (nread != UV_EOF) {
          HLOG(WARNING) << "Read error on stdout, will kill the process: " << uv_strerror(nread);
          Kill();
        } else {
          ClosePipe(kStdout);
        }
    ...
    UV_READ_CB_FOR_CLASS(Subprocess, ReadStderr) {
      ...
      if (nread < 0) {
        if (nread != UV_EOF) {
          HLOG(WARNING) << "Read error on stderr, will kill the process: " << uv_strerror(nread);
          Kill();
        } else {
          ClosePipe(kStderr);
        }
