...
#define NEW_EMPTY_FUNC_CALL(var) \
  ...
  memset(&var, 0, sizeof(FuncCall))
    ...
    inline FuncCall NewFuncCallWithMethod(uint16_t func_id, uint16_t method_id, uint16_t client_id, uint32_t call_id) {
      NEW_EMPTY_FUNC_CALL(func_call);
      func_call.func_id = func_id;
    ...
    inline std::string FuncCallDebugString(const FuncCall& func_call) {
      if (func_call.method_id == 0) {
        return fmt::format("func_id={}, client_id={}, call_id={}", func_call.func_id, func_call.client_id,
                           func_call.call_id);
      } else {
        return fmt::format("func_id={}, method_id={}, client_id={}, call_id={}", func_call.func_id, func_call.method_id,
...
#undef NEW_EMPTY_FUNC_CALL
    ...
    struct Message {
      ...
      char inline_data[__FAAS_MESSAGE_SIZE - __FAAS_CACHE_LINE_SIZE] __attribute__((aligned(__FAAS_CACHE_LINE_SIZE)));
    };
...
#define MESSAGE_INLINE_DATA_SIZE (__FAAS_MESSAGE_SIZE - __FAAS_CACHE_LINE_SIZE)
    ...
    inline FuncCall GetFuncCallFromMessage(const Message& message) {
      DCHECK(IsInvokeFuncMessage(message) || IsDispatchFuncCallMessage(message) || IsFuncCallCompleteMessage(message) ||
    ...
    inline FuncCall GetFuncCallFromMessage(const GatewayMessage& message) {
      DCHECK(IsDispatchFuncCallMessage(message) || IsFuncCallCompleteMessage(message) ||
             IsFuncCallFailedMessage(message));
      FuncCall func_call;
      func_call.func_id = message.func_id;
      func_call.method_id = message.method_id;
    ...
    inline std::span<const char> GetInlineDataFromMessage(const Message& message) {
      if (IsInvokeFuncMessage(message) || IsDispatchFuncCallMessage(message) || IsFuncCallCompleteMessage(message) ||
          IsLauncherHandshakeMessage(message)) {
        if (message.payload_size > 0) {
          return std::span<const char>(message.inline_data, gsl::narrow_cast<size_t>(message.payload_size));
        }
      }
...
#define NEW_EMPTY_GATEWAY_MESSAGE(var) \
  ...
  memset(&var, 0, sizeof(GatewayMessage))
    ...
    inline GatewayMessage NewFuncCallCompleteGatewayMessage(const FuncCall& func_call, int32_t processing_time) {
      NEW_EMPTY_GATEWAY_MESSAGE(message);
      message.message_type = static_cast<uint16_t>(MessageType::FUNC_CALL_COMPLETE);
      SetFuncCallInMessage(&message, func_call);
      message.processing_time = processing_time;
    ...
    inline GatewayMessage NewFuncCallFailedGatewayMessage(const FuncCall& func_call, int32_t status_code = 0) {
      NEW_EMPTY_GATEWAY_MESSAGE(message);
      message.message_type = static_cast<uint16_t>(MessageType::FUNC_CALL_FAILED);
      SetFuncCallInMessage(&message, func_call);
