    ...
    try {                                                                              \
      ...
      if (std::type_index(r) != std::type_index(typeid(ClassName))) {                  \
        ...
        if (reinterpret_cast<void*>(c_ptr) != ptr) {                                   \
          LOG(FATAL) << #ptr << " does not store an instance of class " << #ClassName; \
        }                                                                              \
      }                                                                                \
...
#define UV_CHECK_OK(UV_CALL)                                              \
  do {                                                                    \
    int ret = UV_CALL;                                                    \
    LOG_IF(FATAL, ret != 0) << "libuv call failed: " << uv_strerror(ret); \
  } while (0)

#define UV_DCHECK_OK(UV_CALL)                                              \
  do {                                                                     \
    int ret = UV_CALL;                                                     \
    DLOG_IF(FATAL, ret != 0) << "libuv call failed: " << uv_strerror(ret); \
  } while (0)
...
#define DECLARE_UV_READ_CB_FOR_CLASS(FnName)           \
  ...
  static void FnName##Callback(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf);

#define UV_READ_CB_FOR_CLASS(ClassName, FnName)                                               \
  void ClassName::FnName##Callback(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf) { \
...
#define DECLARE_UV_ALLOC_CB_FOR_CLASS(FnName)            \
  ...
  static void FnName##Callback(uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf);

#define UV_ALLOC_CB_FOR_CLASS(ClassName, FnName)                                                \
  void ClassName::FnName##Callback(uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf) { \
...
#define DECLARE_UV_EXIT_CB_FOR_CLASS(FnName)             \
  ...
  static void FnName##Callback(uv_process_t* process, int64_t exit_status, int term_signal);

#define UV_EXIT_CB_FOR_CLASS(ClassName, FnName)                                                   \
  void ClassName::FnName##Callback(uv_process_t* process, int64_t exit_status, int term_signal) { \
