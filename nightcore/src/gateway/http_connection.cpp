...
namespace faas {
  namespace gateway {
    ...
    void HttpConnection::StartRecvData() {
      ...
      UV_DCHECK_OK(uv_read_start(UV_AS_STREAM(&uv_tcp_handle_), &HttpConnection::BufferAllocCallback,
    ...
    namespace {
      static bool ReadParsedUrlField(const http_parser_url* parsed_url, http_parser_url_fields field,
        ...
        if ((parsed_url->field_set & (1 << field)) == 0) {
          ...
          *result = std::string_view(url_buf + parsed_url->field_data[field].off, parsed_url->field_data[field].len);
          return true;
        }
      }
    ...
    void HttpConnection::OnNewHttpRequest(std::string_view method, std::string_view path, std::string_view qs) {
      DCHECK_IN_EVENT_LOOP_THREAD(uv_tcp_handle_.loop);
      ...
      if (func_entry->qs_as_input) {
        ...
        func_call_context_.append_input(std::span<const char>(encoded_json.data(), encoded_json.length()));
      } else {
    ...
    void HttpConnection::OnFuncCallFinished(FuncCallContext* func_call_context) {
      ...
      io_worker_->ScheduleFunction(this, absl::bind_front(&HttpConnection::OnFuncCallFinishedInternal, this));
    }
    ...
    void HttpConnection::SendHttpResponse(HttpStatus status, std::span<const char> body) {
      ...
      response_header_ = fmt::format(
          ...
          "Content-Length: {}\r\n"
          "\r\n",
          GetHttpStatusString(status), absl::FormatTime(absl::RFC1123_full, absl::Now(), absl::UTCTimeZone()),
          kServerString, kResponseContentType, body.size());
      if (body.size() > 0) {
        uv_buf_t bufs[] = {{.base = const_cast<char*>(response_header_.data()), .len = response_header_.length()},
                           {.base = const_cast<char*>(body.data()), .len = body.size()}};
        UV_DCHECK_OK(uv_write(&response_write_req_, UV_AS_STREAM(&uv_tcp_handle_), bufs, 2,
                              &HttpConnection::DataWrittenCallback));
      } else {
        uv_buf_t buf = {.base = const_cast<char*>(response_header_.data()), .len = response_header_.length()};
        UV_DCHECK_OK(uv_write(&response_write_req_, UV_AS_STREAM(&uv_tcp_handle_), &buf, 1,
                              &HttpConnection::DataWrittenCallback));
      }
    }
