  ...
  namespace gateway {
    ...
    class Server final : public server::ServerBase {
     public:
      ...
      void set_func_config_file(std::string_view path) { func_config_file_ = std::string(path); }
      ...
      void OnRecvEngineMessage(EngineConnection* connection, const protocol::GatewayMessage& message,
     ...
     private:
      ...
      absl::flat_hash_map</* func_id */ uint16_t, size_t> next_dispatch_node_idx_ ABSL_GUARDED_BY(mu_);
      absl::flat_hash_map</* func_id */ uint16_t, size_t> inflight_requests_per_node_ ABSL_GUARDED_BY(mu_);
      absl::flat_hash_map</* full_call_id */ uint64_t, FuncCallState> running_func_calls_ ABSL_GUARDED_BY(mu_);
      std::queue<FuncCallState> pending_func_calls_ ABSL_GUARDED_BY(mu_);
      absl::flat_hash_set</* full_call_id */ uint64_t> discarded_func_calls_ ABSL_GUARDED_BY(mu_);
      absl::flat_hash_map</* connection_id */ int, std::shared_ptr<server::ConnectionBase>> connections_
          ABSL_GUARDED_BY(mu_);
      ...
      absl::flat_hash_map</* func_id */ uint16_t, std::unique_ptr<PerFuncStat>> per_func_stats_ ABSL_GUARDED_BY(mu_);
      ...
      void FinishFuncCall(std::shared_ptr<server::ConnectionBase> parent_connection,
                          FuncCallContext* func_call_context);
      void TickNewFuncCall(uint16_t func_id, int64_t current_timestamp) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_);
