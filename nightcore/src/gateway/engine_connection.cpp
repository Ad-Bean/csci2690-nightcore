...
ABSL_FLAG(bool, engine_conn_enable_nodelay, true, "Enable TCP_NODELAY for connections to engines");
ABSL_FLAG(bool, engine_conn_enable_keepalive, true, "Enable TCP keep-alive for connections to engines");
...
namespace faas {
  namespace gateway {
    ...
    void EngineConnection::Start(server::IOWorker* io_worker) {
      ...
      UV_DCHECK_OK(uv_read_start(UV_AS_STREAM(&uv_tcp_handle_), &EngineConnection::BufferAllocCallback,
    ...
    void EngineConnection::SendMessage(const GatewayMessage& message, std::span<const char> payload) {
      ...
      while (pos < sizeof(GatewayMessage) + payload.size()) {
        ...
        UV_DCHECK_OK(uv_write(write_req, UV_AS_STREAM(&uv_tcp_handle_), &buf, 1, &EngineConnection::DataSentCallback));
        pos += write_size;
      }
    }
    ...
    */
    ...
    UV_READ_CB_FOR_CLASS(EngineConnection, RecvData) {
      ...
      if (nread < 0) {
        if (nread == UV_EOF) {
          ...
          HLOG(ERROR) << "Read error, will close this connection: " << uv_strerror(nread);
        }
        ScheduleClose();
        return;
      }
    ...
    UV_WRITE_CB_FOR_CLASS(EngineConnection, DataSent) {
      ...
      if (status != 0) {
        HLOG(ERROR) << "Failed to send data, will close this connection: " << uv_strerror(status);
        ScheduleClose();
      }
    }
