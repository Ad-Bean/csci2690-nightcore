  ...
  namespace gateway {
    ...
    Server::Server()
        : engine_conn_port_(-1),
          ...
          incoming_requests_stat_(stat::Counter::StandardReportCallback("incoming_requests")),
          request_interval_stat_(stat::StatisticsCollector<int32_t>::StandardReportCallback("request_interval")),
          requests_instant_rps_stat_(stat::StatisticsCollector<float>::StandardReportCallback("requests_instant_rps")),
          inflight_requests_stat_(stat::StatisticsCollector<uint16_t>::StandardReportCallback("inflight_requests")),
          running_requests_stat_(stat::StatisticsCollector<uint16_t>::StandardReportCallback("running_requests")),
          queueing_delay_stat_(stat::StatisticsCollector<int32_t>::StandardReportCallback("queueing_delay")),
          dispatch_overhead_stat_(stat::StatisticsCollector<int32_t>::StandardReportCallback("dispatch_overhead")) {
    ...
    void Server::StartInternal() {
      ...
      HLOG(INFO) << fmt::format("Listen on {}:{} for engine connections", address_, engine_conn_port_);
      UV_CHECK_OK(uv_listen(UV_AS_STREAM(&uv_engine_conn_handle_), listen_backlog_, &Server::EngineConnectionCallback));
      ...
      UV_CHECK_OK(uv_listen(UV_AS_STREAM(&uv_http_handle_), listen_backlog_, &Server::HttpConnectionCallback));
      ...
      UV_CHECK_OK(uv_listen(UV_AS_STREAM(&uv_grpc_handle_), listen_backlog_, &Server::GrpcConnectionCallback));
    }
    ...
    void Server::OnConnectionClose(server::ConnectionBase* connection) {
      ...
      if (connection->type() == HttpConnection::kTypeId || connection->type() == GrpcConnection::kTypeId) {
    ...
    void Server::OnNewGrpcFuncCall(GrpcConnection* connection, FuncCallContext* func_call_context) {
      ...
      FuncCall func_call =
          NewFuncCallWithMethod(gsl::narrow_cast<uint16_t>(func_entry->func_id),
        ...
        uint16_t node_id = 0;
        {
          ...
          if (running_func_calls_.contains(func_call.full_call_id)) {
            ...
            if (next_func_call != nullptr) {
              ...
              queueing_delay_stat_.AddSample(gsl::narrow_cast<int32_t>(current_timestamp - state.recv_timestamp));
              running_func_calls_[func_call.full_call_id] = std::move(state);
              node_id = PickNextNode(func_call);
              running_requests_stat_.AddSample(gsl::narrow_cast<uint16_t>(running_func_calls_.size()));
            }
          }
        }
    ...
    Server::PerFuncStat::PerFuncStat(uint16_t func_id)
        : last_request_timestamp(-1),
          incoming_requests_stat(stat::Counter::StandardReportCallback(fmt::format("incoming_requests[{}]", func_id))),
    ...
    void Server::TickNewFuncCall(uint16_t func_id, int64_t current_timestamp) {
      ...
      if (last_request_timestamp_ != -1) {
        per_func_stat->request_interval_stat.AddSample(
            gsl::narrow_cast<int32_t>(current_timestamp - per_func_stat->last_request_timestamp));
      }
    ...
    uint16_t Server::PickNextNode(const protocol::FuncCall& func_call) {
      ...
      if (absl::GetFlag(FLAGS_lb_per_fn_round_robin)) {
        ...
        for (size_t i = 1; i < connected_nodes_.size(); i++) {
          if (inflight_requests_per_node_[connected_nodes_[i]] < inflight_requests_per_node_[connected_nodes_[idx]]) {
            idx = i;
          }
        }
      } else {
      ...
      bool no_connected_nodes = false;
      {
        ...
        if (last_request_timestamp_ != -1) {
          requests_instant_rps_stat_.AddSample(
              gsl::narrow_cast<float>(1e6 / (current_timestamp - last_request_timestamp_)));
          request_interval_stat_.AddSample(gsl::narrow_cast<int32_t>(current_timestamp - last_request_timestamp_));
        }
        ...
        if (connected_nodes_.size() == 0) {
          ...
          FuncCallState state = {.func_call = func_call,
    ...
    void Server::DispatchFuncCall(std::shared_ptr<server::ConnectionBase> parent_connection,
      ...
      server::ConnectionBase* engine_connection = io_worker->PickConnection(EngineConnection::type_id(node_id));
      if (engine_connection != nullptr) {
        ...
        engine_connection->as_ptr<EngineConnection>()->SendMessage(dispatch_message, func_call_context->input());
      } else {
    ...
    bool Server::OnEngineHandshake(uv_tcp_t* uv_handle, std::span<const char> data) {
      ...
      std::span<const char> remaining_data(data.data() + sizeof(GatewayMessage), data.size() - sizeof(GatewayMessage));
      std::shared_ptr<server::ConnectionBase> connection(new EngineConnection(this, node_id, conn_id, remaining_data));
      if (!connected_node_set_.contains(node_id)) {
        connected_node_set_.insert(node_id);
        absl::MutexLock lk(&mu_);
        connected_nodes_.push_back(node_id);
        dispatched_requests_stat_.emplace_back(
            new stat::Counter(stat::Counter::StandardReportCallback(fmt::format("dispatched_requests[{}]", node_id))));
      ...
      HLOG(INFO) << fmt::format("New engine connection (node_id={}, conn_id={}) assigned to IO worker {}", node_id,
                                conn_id, worker_id);
    ...
    UV_CONNECTION_CB_FOR_CLASS(Server, HttpConnection) {
      ...
      std::shared_ptr<server::ConnectionBase> connection(new HttpConnection(this, next_http_connection_id_++));
    ...
    UV_CONNECTION_CB_FOR_CLASS(Server, GrpcConnection) {
      ...
      std::shared_ptr<server::ConnectionBase> connection(new GrpcConnection(this, next_grpc_connection_id_++));
      uv_tcp_t* client = reinterpret_cast<uv_tcp_t*>(malloc(sizeof(uv_tcp_t)));
      UV_DCHECK_OK(uv_tcp_init(uv_loop(), client));
    ...
    UV_CONNECTION_CB_FOR_CLASS(Server, EngineConnection) {
      ...
      if (uv_accept(UV_AS_STREAM(&uv_engine_conn_handle_), UV_AS_STREAM(client)) == 0) {
        ongoing_engine_handshakes_.insert(
            std::unique_ptr<OngoingEngineHandshake>(new OngoingEngineHandshake(this, client)));
      } else {
    ...
    Server::OngoingEngineHandshake::OngoingEngineHandshake(Server* server, uv_tcp_t* uv_handle)
      ...
      UV_DCHECK_OK(uv_read_start(UV_AS_STREAM(uv_handle_), &OngoingEngineHandshake::BufferAllocCallback,
