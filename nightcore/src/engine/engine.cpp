  ...
  namespace engine {
    ...
    Engine::Engine()
        : gateway_port_(-1),
          ...
          incoming_external_requests_stat_(stat::Counter::StandardReportCallback("incoming_external_requests")),
          incoming_internal_requests_stat_(stat::Counter::StandardReportCallback("incoming_internal_requests")),
          ...
          message_delay_stat_(stat::StatisticsCollector<int32_t>::StandardReportCallback("message_delay")),
    ...
    void Engine::StartInternal() {
      ...
      for (int i = 0; i < total_gateway_conn; i++) {
        ...
        UV_CHECK_OK(uv_tcp_connect(req, uv_handle, (const struct sockaddr*)&addr, &Engine::GatewayConnectCallback));
      }
      // Listen on ipc_path
    ...
    void Engine::OnConnectionClose(server::ConnectionBase* connection) {
      ...
      if (connection->type() == MessageConnection::kTypeId) {
        ...
        HLOG(WARNING) << fmt::format("Gateway connection (conn_id={}) disconencted", gateway_connection->conn_id());
    ...
    bool Engine::OnNewHandshake(MessageConnection* connection, const Message& handshake_message, Message* response,
                                std::span<const char>* response_payload) {
      if (!IsLauncherHandshakeMessage(handshake_message) && !IsFuncWorkerHandshakeMessage(handshake_message)) {
    ...
    void Engine::OnRecvMessage(MessageConnection* connection, const Message& message) {
      ...
      if (IsInvokeFuncMessage(message)) {
        ...
        if (dispatcher != nullptr) {
          if (message.payload_size < 0) {
            success = dispatcher->OnNewFuncCall(func_call, parent_func_call,
        ...
        if (dispatcher != nullptr) {
          if (IsFuncCallCompleteMessage(message)) {
            ...
            if (success && func_call.client_id == 0) {
              if (message.payload_size < 0) {
                auto output_region = ipc::ShmOpen(ipc::GetFuncCallOutputShmName(func_call.full_call_id));
                if (output_region == nullptr) {
                  ExternalFuncCallFailed(func_call);
                } else {
                  output_region->EnableRemoveOnDestruction();
                  ExternalFuncCallCompleted(func_call, output_region->to_span(), message.processing_time);
                }
              } else {
                ExternalFuncCallCompleted(func_call, GetInlineDataFromMessage(message), message.processing_time);
              }
            }
          } else {
    ...
    void Engine::OnExternalFuncCall(const FuncCall& func_call, std::span<const char> input) {
      ...
      if (input.size() > MESSAGE_INLINE_DATA_SIZE) {
        input_region = ipc::ShmCreate(ipc::GetFuncCallInputShmName(func_call.full_call_id), input.size());
      ...
      Dispatcher* dispatcher = nullptr;
      {
        ...
        if (last_external_request_timestamp_ != -1) {
          external_requests_instant_rps_stat_.AddSample(
              gsl::narrow_cast<float>(1e6 / (current_timestamp - last_external_request_timestamp_)));
        }
        last_external_request_timestamp_ = current_timestamp;
        inflight_external_requests_stat_.AddSample(gsl::narrow_cast<uint16_t>(inflight_external_requests_.load()));
        dispatcher = GetOrCreateDispatcherLocked(func_call.func_id);
        if (input_region != nullptr) {
          if (dispatcher != nullptr) {
      ...
      bool success = false;
      if (input.size() <= MESSAGE_INLINE_DATA_SIZE) {
        success = dispatcher->OnNewFuncCall(func_call, protocol::kInvalidFuncCall, input.size(),
                                            /* inline_input= */ input, /* shm_input= */ false);
      } else {
        success = dispatcher->OnNewFuncCall(func_call, protocol::kInvalidFuncCall, input.size(),
                                            /* inline_input= */ std::span<const char>(), /* shm_input= */ true);
      }
      if (!success) {
        {
          absl::MutexLock lk(&mu_);
    ...
    void Engine::ExternalFuncCallCompleted(const protocol::FuncCall& func_call, std::span<const char> output,
                                           int32_t processing_time) {
      inflight_external_requests_.fetch_add(-1);
      server::IOWorker* io_worker = server::IOWorker::current();
      DCHECK(io_worker != nullptr);
      server::ConnectionBase* gateway_connection = io_worker->PickConnection(GatewayConnection::kTypeId);
    ...
    void Engine::ProcessDiscardedFuncCallIfNecessary() {
      ...
      if (!discarded_internal_func_calls.empty()) {
        ...
        for (const FuncCall& func_call : discarded_internal_func_calls) {
          if (use_fifo_for_nested_call_) {
            worker_lib::FifoFuncCallFinished(func_call, /* success= */ false, /* output= */ std::span<const char>(),
    ...
    UV_CONNECT_CB_FOR_CLASS(Engine, GatewayConnect) {
      ...
      HLOG(INFO) << fmt::format("New gateway connection (conn_id={}) assigned to IO worker {}", conn_id,
                                next_gateway_conn_worker_id_);
