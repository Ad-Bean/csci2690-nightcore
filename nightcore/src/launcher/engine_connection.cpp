...
namespace faas {
  namespace launcher {
    ...
    void EngineConnection::Start(uv_loop_t* uv_loop, int engine_tcp_port, const Message& handshake_message) {
      ...
      if (engine_tcp_port == -1) {
        ...
        uv_pipe_connect(&connect_req_, pipe_handle, std::string(ipc::GetEngineUnixSocketPath()).c_str(),
        ...
        uv_tcp_connect(&connect_req_, tcp_handle, (const struct sockaddr*)&addr, &EngineConnection::ConnectCallback);
    ...
    void EngineConnection::RecvHandshakeResponse() {
      ...
      if (launcher_->OnRecvHandshakeResponse(*response, payload)) {
        ...
        UV_DCHECK_OK(uv_read_start(engine_conn_, &EngineConnection::BufferAllocCallback,
    ...
    void EngineConnection::WriteMessage(const Message& message) {
      ...
      UV_DCHECK_OK(uv_write(write_req, engine_conn_, &buf, 1, &EngineConnection::WriteMessageCallback));
    }

    UV_CONNECT_CB_FOR_CLASS(EngineConnection, Connect) {
      if (status != 0) {
        HLOG(WARNING) << "Failed to connect to engine, will close the connection: " << uv_strerror(status);
        ScheduleClose();
        return;
      }
      HLOG(INFO) << "Connected to engine, start writing handshake message";
      uv_buf_t buf = {.base = reinterpret_cast<char*>(&handshake_message_), .len = sizeof(Message)};
      UV_DCHECK_OK(
          uv_write(launcher_->NewWriteRequest(), engine_conn_, &buf, 1, &EngineConnection::WriteHandshakeCallback));
    }
    ...
    UV_READ_CB_FOR_CLASS(EngineConnection, ReadHandshakeResponse) {
      ...
      if (nread < 0) {
        HLOG(WARNING) << "Read error on handshake, will close the connection: " << uv_strerror(nread);
        ScheduleClose();
        return;
      }
      if (nread == 0) {
    ...
    UV_WRITE_CB_FOR_CLASS(EngineConnection, WriteHandshake) {
      auto reclaim_resource = gsl::finally([this, req] { launcher_->ReturnWriteRequest(req); });
      if (status != 0) {
        HLOG(WARNING) << "Failed to write handshake message, will close the connection: " << uv_strerror(status);
        ScheduleClose();
        return;
      }
      UV_DCHECK_OK(uv_read_start(engine_conn_, &EngineConnection::BufferAllocCallback,
    ...
    UV_READ_CB_FOR_CLASS(EngineConnection, ReadMessage) {
      ...
      utils::ReadMessages<Message>(&message_buffer_, buf->base, nread,
    ...
    UV_WRITE_CB_FOR_CLASS(EngineConnection, WriteMessage) {
      ...
      if (status != 0) {
        HLOG(WARNING) << "Failed to write response, will close the connection: " << uv_strerror(status);
        ScheduleClose();
      }
    }
