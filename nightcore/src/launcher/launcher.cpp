  ...
  namespace launcher {
    ...
    Launcher::Launcher()
        : state_(kCreated),
          ...
          engine_tcp_port_(-1),
          event_loop_thread_("Launcher/EL", absl::bind_front(&Launcher::EventLoopThreadMain, this)),
    ...
    void Launcher::Start() {
      ...
      SetInlineDataInMessage(&handshake_message,
                             std::span<const char>(self_container_id.data(), self_container_id.size()));
    ...
    bool Launcher::OnRecvHandshakeResponse(const Message& handshake_response, std::span<const char> payload) {
    ...
    void Launcher::OnRecvMessage(const protocol::Message& message) {
      ...
      if (IsCreateFuncWorkerMessage(message)) {
        if (fprocess_mode_ == kCppMode) {
          auto func_process = std::make_unique<FuncProcess>(this, /* id= */ func_processes_.size(),
        ...
        } else if (fprocess_mode_ == kGoMode || fprocess_mode_ == kNodeJsMode || fprocess_mode_ == kPythonMode) {
          if (func_processes_.empty()) {
            auto func_process =
                std::make_unique<FuncProcess>(this, /* id= */ 0, /* initial_client_id= */ message.client_id);
            if (func_process->Start(&uv_loop_, &buffer_pool_)) {
              func_processes_.push_back(std::move(func_process));
            } else {
              HLOG(FATAL) << "Failed to start function process!";
