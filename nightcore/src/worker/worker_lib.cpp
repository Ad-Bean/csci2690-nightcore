  ...
  namespace worker_lib {
    ...
    namespace {
      ...
      bool WriteOutputToShm(const FuncCall& func_call, std::span<const char> output) {
        auto output_region = ipc::ShmCreate(ipc::GetFuncCallOutputShmName(func_call.full_call_id), output.size());
      ...
      bool WriteOutputToFifo(const FuncCall& func_call, bool success, std::span<const char> output, char* pipe_buf) {
        VLOG(1) << "Start writing output to FIFO";
        int output_fifo =
            ipc::FifoOpenForWrite(ipc::GetFuncCallOutputFifoName(func_call.full_call_id), /* nonblocking= */ true);
        if (output_fifo == -1) {
    ...
    bool GetFuncCallInput(const Message& dispatch_func_call_message, std::span<const char>* input,
    ...
    void FifoFuncCallFinished(const FuncCall& func_call, bool success, std::span<const char> output,
                              int32_t processing_time, char* pipe_buf, Message* response) {
      if (success) {
    ...
    void FuncCallFinished(const protocol::FuncCall& func_call, bool success, std::span<const char> output,
                          int32_t processing_time, protocol::Message* response) {
      if (success) {
        *response = NewFuncCallCompleteMessage(func_call, processing_time);
        if (output.size() <= MESSAGE_INLINE_DATA_SIZE) {
          SetInlineDataInMessage(response, output);
    ...
    bool PrepareNewFuncCall(const FuncCall& func_call, uint64_t parent_func_call, std::span<const char> input,
                            std::unique_ptr<ipc::ShmRegion>* shm_region, Message* invoke_func_message) {
      *invoke_func_message = NewInvokeFuncMessage(func_call, parent_func_call);
      if (input.size() <= MESSAGE_INLINE_DATA_SIZE) {
        ...
        auto input_region = ipc::ShmCreate(ipc::GetFuncCallInputShmName(func_call.full_call_id), input.size());
        if (input_region == nullptr) {
          LOG(ERROR) << "ShmCreate failed";
          return false;
        }
    ...
    bool FifoGetFuncCallOutput(const FuncCall& func_call, int output_fifo_fd, char* pipe_buf, bool* success,
                               std::span<const char>* output, std::unique_ptr<ipc::ShmRegion>* shm_region,
                               bool* pipe_buf_used) {
