  ...
  namespace base {
    ...
    void Thread::MarkThreadCategory(absl::string_view category) {
      ...
      if (!cpuset_str.empty()) {
        ...
        LOG(INFO) << "Does not find cpuset setting for " << category << " threads (can be set by " << cpuset_var_name
                  << ")";
      }
      // Set nice
      ...
      if (!nice_str.empty()) {
        ...
        LOG(INFO) << "Does not find nice setting for " << category << " threads (can be set by " << nice_var_name
                  << ")";
      }
    }
